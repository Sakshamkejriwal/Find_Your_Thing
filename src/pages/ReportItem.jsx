...
import { analyzeFoundItem } from "../utils/ai";
...
function ReportItem() {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        img.onload = () => {
          ...
          // Auto-fill using AI
          if (!isEditing) {
            handleAIAutoFill(compressedDataUrl);
          }
        };
      };
  ...
  const handleAIAutoFill = async (imageData) => {
    setIsAnalyzing(true);
    try {
      const result = await analyzeFoundItem(imageData, CATEGORIES);
      if (result) {
        setFormData(prev => ({
          ...prev,
      ...
      console.warn("AI Auto-fill failed. User will enter manually.");
    } finally {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl border border-gray-100 p-8 sm:p-10 space-y-6 shadow-sm">
          ...
          <div>
            ...
            {isAnalyzing && (
              <div className="mt-4 flex items-center gap-3 text-[#D35400] bg-[#FFF0E6] p-3 rounded-xl animate-pulse">
                <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                <span className="text-sm font-medium italic">Magic Fill is analyzing your photo...</span>
              </div>
            )}
          </div>
